...
f = open("src/output/granularity_analysis"+"_ipv"+ipversion+"_"+collector+"_"+str(year)+"_"+str(year)+".csv", "w")
writer = csv.writer(f)
            ...
            while goNext == False:
                if len(as_path) > i:
                    ...
                    while as_path[-j - 1] == as_path[-j]:  # check prepending done by the stub AS
                        ...
                        j += 1 
                    i+=1
                    if(prepend_length > 40):
                        if as_path[-1] == as_to_analyse:
                            print('AS 254')
                            tobestored[prepend_length] = prepend_dict.get(prepend_length,0) + 1
